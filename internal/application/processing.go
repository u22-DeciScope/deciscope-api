...
func (s *Service) UploadFile(ctx context.Context, workspaceID, filename, mediaType string, src io.Reader) (*UploadResult, error) {
	job, err := s.jobs.CreateJob(ctx, workspaceID, "file.extract_audio", "", "completed")
	if err != nil {
	...
	upload, err := s.uploads.SaveUpload(ctx, workspaceID, filename, mediaType, path, job.ID)
	if err != nil {
