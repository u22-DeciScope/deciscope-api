...
- discourse_only_item、low_information_item、recap_only_item、duplicate_item、superseded_item、empty_group、empty_unclassified_containerを明示的に監査する。「追加論点」「別の問題の存在を確認」「次の話題へ移行」「ここまでをまとめる」のような語句は例にすぎず、固定語句ではなく、独立した事実・判断・問題・行動・担当・期限を持つかで判定する。
