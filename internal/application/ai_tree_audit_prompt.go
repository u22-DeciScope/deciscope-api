...
- 次の17種はサーバーが実際に適用しうるoperation(安全条件を満たさない場合は提案してもfindingとして記録されるだけで不採用): move_item, restore_previous_parent, move_node, merge_items, rewrite_item, rewrite_item_title, rewrite_item_description, reclassify_kind, reclassify_subtype, deactivate_item, assign_item_to_candidate, change_evidence_role, create_topic_from_candidate, fold_candidate_into_topic, deactivate_candidate, rename_group, remove_empty_group。
...
- deactivate_itemで最後のactive childを除く場合は、空になるgroup/dynamic topic/topic-unclassifiedにremove_empty_groupを提案してよい。後者のdependsOnOperationIdsには前者のoperationIdを入れる。fixed agendaとrootは空でも絶対に除去しない。
