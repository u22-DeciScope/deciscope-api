...
func TestLoginCreatesUserAndSessionWithoutWorkspace(t *testing.T) {
	...
	// ワークスペースはログイン時に自動作成しない。所属0件のままログインできる。
	if result.User.Email != "user@example.com" || len(result.Workspaces) != 0 || result.Token == "" {
		t.Fatalf("Login() result = %+v", result)
	}
	if result.Session.CurrentWorkspaceID != "" {
		t.Fatalf("current workspace = %q, want empty", result.Session.CurrentWorkspaceID)
	}
...
func TestLoginAllowsGoogleIdentity(t *testing.T) {
	repository := &fakeRepository{}
	service := appauth.NewService(
		repository,
		fakeVerifier{identity: &appauth.Identity{
			UID: "firebase-google-uid", Email: "user@gmail.com", Name: "User",
			EmailVerified: true, Provider: "google.com",
		}},
		time.Hour,
	)
	...
	result, err := service.Login(context.Background(), "token")
	if err != nil {
		t.Fatalf("Login(google) error = %v", err)
	}
	if result.User.Email != "user@gmail.com" || result.Token == "" {
		t.Fatalf("Login(google) result = %+v", result)
	}
}
