...
type Factory func(t *testing.T) core.Repositories
...
func Run(t *testing.T, factory Factory) {
	...
	t.Run("meetings", func(t *testing.T) {
		...
		if _, err := repos.Meetings.GetMeeting(ctx, "missing"); !errors.Is(err, core.ErrNotFound) {
			t.Fatalf("GetMeeting(missing) error = %v, want ErrNotFound", err)
		}
		if err := repos.Meetings.ResetMeeting(ctx, "missing"); !errors.Is(err, core.ErrNotFound) {
	...
	t.Run("events and reset", func(t *testing.T) {
		...
		partial, err := repos.Events.AppendEvent(ctx, meeting.ID, core.EventTranscriptPartial, map[string]any{"text": "draft"})
		if err != nil {
		...
		final, err := repos.Events.AppendEvent(ctx, meeting.ID, core.EventTranscriptFinal, map[string]any{
			"segment_id":    "seg_001",
		...
		state, err := repos.Events.AppendEvent(ctx, meeting.ID, core.EventMeetingState, map[string]any{"status": "ended"})
		if err != nil {
	...
	t.Run("reports", func(t *testing.T) {
		...
		if _, err := repos.Reports.LatestReport(ctx, meeting.ID); !errors.Is(err, core.ErrNotFound) {
	...
	t.Run("jobs and uploads", func(t *testing.T) {
		...
		if _, err := repos.Jobs.GetJob(ctx, "missing"); !errors.Is(err, core.ErrNotFound) {
			t.Fatalf("GetJob(missing) error = %v, want ErrNotFound", err)
		}
	})
}

func createMeeting(t *testing.T, ctx context.Context, repos core.Repositories) *core.Meeting {
	t.Helper()
