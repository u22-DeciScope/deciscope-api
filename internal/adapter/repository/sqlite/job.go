...
func (s *Store) CompleteJob(ctx context.Context, jobID string, result any) error {
	...
	res, err := s.db.ExecContext(ctx, `UPDATE jobs SET status = 'completed', result = ?, error = NULL, updated_at = ? WHERE id = ?`, string(resultBytes), time.Now().UTC().Format(time.RFC3339), jobID)
	if err != nil {
		return err
	}
...
func (s *Store) FailJob(ctx context.Context, jobID, message string) error {
	res, err := s.db.ExecContext(ctx, `UPDATE jobs SET status = 'failed', error = ?, updated_at = ? WHERE id = ?`, message, time.Now().UTC().Format(time.RFC3339), jobID)
	if err != nil {
		return err
	}
	if affected, _ := res.RowsAffected(); affected == 0 {
