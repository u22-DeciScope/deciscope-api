...
func resetTestDatabase(t *testing.T, db *sql.DB) {
	t.Helper()
	_, err := db.Exec(`
		TRUNCATE TABLE meeting_tree_audit_runs, meeting_session_ai_analyses, meeting_session_ai_analysis_live_history, meeting_session_agenda_progress_overrides, transcript_segments, meeting_sessions, uploads, jobs, meeting_reports, meeting_segments, meeting_events,
