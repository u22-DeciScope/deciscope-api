...
type EventStore interface {
	ListEvents(ctx context.Context, meetingID string, afterSeq int64) ([]application.Event, error)
...
func (h *Hub) Publish(event application.Event) {
	h.mu.RLock()
...
func (h *Hub) ServeWS(store EventStore) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		c := &client{
			...
			conn:      conn,
			reader:    reader,
			send:      make(chan application.Event, 128),
...
type client struct {
	...
	conn      netConn
	reader    frameReader
	send      chan application.Event
	done      chan struct{}
	writeMu   sync.Mutex
	lastSeq   int64
}

func (c *client) enqueue(event application.Event) {
	select {
	case c.send <- event:
	default:
		select {
...
func (c *client) writeEvent(event application.Event) error {
