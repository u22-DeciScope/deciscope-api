...
func TestCoreAPIHTTPContract(t *testing.T) {
	service := application.NewService(memory.Repositories(memory.NewMemoryStore()), nil, storage.NewLocal(t.TempDir()))
	api := NewCoreAPI(service, fixture.NewManager(service, t.TempDir()))
	...
	var meeting application.Meeting
